...
//		tu_string proxy = get_proxy_addr();
	...
	bool tcp::connect()
	{
		...
		if (m_socket == INVALID_SOCKET)
		{
			myprintf("can't open listen socket\n");
			return false;
		}
		...
		if (a[0] >= 0 && a[1] >= 0 && a[2] >= 0 && a[3] >= 0)	// absolue address ?
		{
			Uint8 b[4];
			b[0] = a[0];
			b[1] = a[1];
		...
		else
		{
			...
			if (he == NULL)
			{
				...
				myprintf("host '%s' is unreachable, errcode=%d\n", m_host.c_str(), er);
				close();
				return false;
			}
			
			// get server address
			memcpy(&m_saddr.sin_addr, he->h_addr, he->h_length);
		}

		set_nonblock();

		//::connect(m_socket, (struct sockaddr*) &m_saddr, sizeof(struct sockaddr));
		return is_connected();
	}
	...
	bool tcp::is_connected()
	{
		...
		::connect(m_socket, (struct sockaddr*) &m_saddr, sizeof(struct sockaddr));
		int err = WSAGetLastError();
		//printf("is_connected: %d, socket=%d\n", err, m_socket);
