	...
	void as_netstream::run()
	{
		...
		while (m_status != CLOSE)
		{
			...
			if (m_seek_time >= 0)
			{
				...
				int ret = avformat_seek_file(m_FormatCtx, -1, m_seek_time, m_seek_time, m_seek_time, AVSEEK_FLAG_BYTE);
				if (ret == 0)
				{
					m_aq.clear();
					m_vq.clear();
				...
				if (m_ACodecCtx->sample_fmt == AV_SAMPLE_FMT_FLTP)
				{
					...
					for (int i = 0; i < frame.nb_samples; i++)
					{
						for (int c = 0; c < frame.channels; c++)
						{
							...
							*pbuf++ = fclamp(extended_data[i], -1.0f, 1.0f) * 32767.0f;
						}
					}
