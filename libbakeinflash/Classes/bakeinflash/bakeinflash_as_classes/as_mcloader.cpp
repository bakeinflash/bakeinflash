...
namespace bakeinflash
{
	...
	void as_mcloader::set_target(int index, tu_file* in)
	{
		...
		if (memcmp(signature, "CWS", 3) == 0 || memcmp(signature, "C++", 3) == 0 || memcmp(signature, "FWS", 3) == 0)
		{
			...
			if (md && md->get_frame_count() > 0 && m_ld[index]->m_target != NULL)
			{
				m_ld[index]->m_target->replace_me(md);
				handle(index, onLoadInit);
			}
			else
			{
				if (m_ld[index]->m_target == NULL)
				{
		...
		if (memcmp(signature, "\xFF\xD8\xFF", 3) == 0)
		{
			...
			if (m_ld[index]->m_target != NULL && im)
			{
				...
				m_ld[index]->m_target->replace_me(jpeg);
				handle(index, onLoadInit);
			}
			delete in;
		}
