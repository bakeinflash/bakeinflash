...
#define DO_STEREO_BLOCK(out_data, n_bits, sample_count, in, left_sample, left_stepsize_index, right_sample, right_stepsize_index) \
  ...
  void	adpcm_expand(
    ...
    bool stereo)
    // Utility function: uncompress ADPCM data from in stream to out_data[].
    // The output buffer must have (sample_count*2) bytes for mono, or (sample_count*4) bytes for stereo.
  {
