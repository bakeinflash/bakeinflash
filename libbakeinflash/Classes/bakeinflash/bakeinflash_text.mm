...
namespace bakeinflash
{
	...
	// Draw the string.
	{
		...
		//	float scale = fmin(xscale, yscale);
			...
			// keep bound's ratio
			m_text_rendered->m_xratio = w / (float) p2(w);
			m_text_rendered->m_yratio = h / (float) p2(h);
		}

		if (m_text_rendered != NULL)
		{
			rect uv_bounds(0, m_text_rendered->m_xratio, 0, m_text_rendered->m_yratio);
...
#if iOS == 1
    myTextField *textField = (myTextField*) et->m_ios_field;
    if (textField && fn.nargs > 0)
    {
      const tu_string& str = fn.arg(0).to_tu_string();
      int kbd;
