...
//		fontlib::output_cached_data(out, fonts, this, options);
	...
	// Read in cached data and use it to prime our loaded characters.
	{
		...
		{
			myprintf("cache file does not have the correct format; skipping\n");
			return;
		}
...
//		fontlib::input_cached_data(in, fonts, this);
		...
		for (;;)
		{
			if (in->get_error() != TU_FILE_NO_ERROR)
			{
				myprintf("error reading cache file (characters); skipping\n");
				return;
			}
			if (in->get_eof())
			{
				myprintf("unexpected eof reading cache file (characters); skipping\n");
				return;
			}
			...
			{
				myprintf("sync error in cache file (reading characters)!  "
