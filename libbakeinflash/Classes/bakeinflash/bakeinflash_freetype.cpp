...
namespace bakeinflash
{
    ...
    tu_string url = key + ".ttf";
		...
		{
			url = tu_string(get_workdir()) + key + ".ttf";
			FT_Error er = FT_New_Face(m_lib, url.c_str(), 0, &face);
			if (er)
			{
				static string_hash<bool> s_printed_names;
				if (s_printed_names.get(url, NULL) == false)
				{
					myprintf("could not open '%s'\n", url.c_str());
					s_printed_names[url] = true;

					url = tu_string(get_workdir()) + "default.ttf";
					myprintf("using '%s'\n", url.c_str());
				}
				url = tu_string(get_workdir()) + "default.ttf";
