...
//
#if defined(_MSC_VER) && _MSC_VER >= 1400 && !defined( __EMSCRIPTEN___)
#include <vld.h>
#endif
...
// For handling notification callbacks from ActionScript.
{
  ...
  else if (stricmp(command, "set_max_volume") == 0)
  {
  ...
  else if (stricmp(command, "notify_keydown") == 0)
  {
  ...
  else if (stricmp(command, "notify_keyup") == 0)
  {
    Uint16 ch = utf8::decode_next_unicode_character(&args);
    key::code c = translate_key(ch);
    if (c != key::INVALID)
  ...
  else if (stricmp(command, "clear_events") == 0)
  {
  ...
  else if (stricmp(command, "set_workdir") == 0)
  {
    set_workdir(args);
  }
  else if (stricmp(command, "fps") == 0)
  {
    root* rm = get_root();
    int fps = atoi(args);
    rm->set_frame_rate((float)fps);
  }
  else if (stricmp(command, "resetSound") == 0)
  {
    delete sh;
#ifdef TU_USE_SDL_SOUND
...
int	main(int argc, char* argv[])
{
  ...
  rh = create_render_handler_ogles2();
  ...
  int w = m->get_movie_width();
  int h = m->get_movie_height();
  int screen_height = h;
  int screen_width = w;
  ...
  float scale_x = (float)screen_width / w;
  float scale_y = (float)screen_height / h;
  scale = fmin(scale_x, scale_y);

  // move movie to center
  int view_width = (int)(width * scale);
  int view_height = (int)(height * scale);
  ...
  SDL_GL_CreateContext(s_screen); // создаем контекст OpenGL
...
//	glHint(GL_LINE_SMOOTH_HINT, GL_NICEST);	// GL_NICEST, GL_FASTEST, GL_DONT_CARE
  ...
  loop = [&]
  {
    ...
    // Don't hog the CPU.
    float ms = 1000.0f / m->get_frame_rate();
    ms -= t_advance + t_draw;
    ms = fmax(1, ms);
    tu_timer::sleep(ms);
#endif
    ...
    //myprintf("loop time: %d, swapBuffer time: %d, advance time: %d, display time %d, sleep %d\n",	tu_timer::get_ticks() - ticks, t_draw, t_advance, t_display, (int)ms);
